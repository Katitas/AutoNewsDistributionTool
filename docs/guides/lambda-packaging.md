...
これだけで以下が自動実行される：
- arm64 wheel で `requirements.txt` の依存をインストール（`BuildMethod: python3.12`）
...
| `AccessDeniedException` (Bedrock) | モデル ARN が IAM ポリシーと不一致 | CFn パラメータ `BedrockModelArn` を確認 |
